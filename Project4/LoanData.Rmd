...
credit_grade_group <- group_by(loans, CreditGrade)
loans.cg_group <- summarise(credit_grade_group,
                            credit_score_mean = mean(CreditScoreRangeLower),
                            credit_score_median = median(CreditScoreRangeLower),
                            n = n())

loans.cg_group <- arrange(loans.cg_group, CreditGrade)
...
ggplot(aes(x = CreditScoreRangeUpper, y = DebtToIncomeRatio), data = loans) +
  coord_cartesian(xlim = c(500, 900), ylim = c(0, 1), expand = "FALSE") +
...
ggplot(aes(x = listDate, y = CreditScoreRangeLower), data = loans) +
  geom_jitter(aes(colour = LoanStatus), alpha = 0.5) +
  geom_smooth() +
...
ggplot(aes(x = IncomeRange, y = LoanOriginalAmount, color = Term), data = loans) +
  ...
  labs(x = "Income Range", y = "Original Loan Amount", color = "Loan Term") +
  theme_dark()
```
